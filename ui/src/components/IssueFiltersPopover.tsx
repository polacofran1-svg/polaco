  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "border-accent/25 bg-accent/10 text-accent dark:text-accent" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? `Filters: ${activeFilterCount}` : "Filter") : undefined}>
          ...
          {iconOnly && activeFilterCount > 0 ? <span className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-accent text-[9px] font-bold text-accent-foreground">{activeFilterCount}</span> : null}
      ...
      <PopoverContent
        align="end"
        className="w-[min(780px,calc(100vw-2rem))] max-h-[min(80vh,42rem)] overflow-y-auto overscroll-contain rounded-[1.5rem] p-0"
      >
        <div className="space-y-4 p-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold tracking-[-0.01em]">Filters</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
          ...
          <div className="space-y-1.5">
            <span className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Quick filters</span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                ...
                <div className="space-y-0.5">
                  {issueStatusOrder.map((status) => (
                    <label key={status} className="flex cursor-pointer items-center gap-2 rounded-xl px-2 py-1.5 hover:bg-secondary">
                      <Checkbox
              ...
              <div className="space-y-1">
                <span className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Priority</span>
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-xl px-2 py-1.5 hover:bg-secondary">
                      <Checkbox
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Assignee</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-xl px-2 py-1.5 hover:bg-secondary">
                    <Checkbox
                  ...
                  {(agents ?? []).map((agent) => (
                    <label key={agent.id} className="flex cursor-pointer items-center gap-2 rounded-xl px-2 py-1.5 hover:bg-secondary">
                      <Checkbox
