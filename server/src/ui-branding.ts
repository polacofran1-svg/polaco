...
function createFaviconDataUrl(background: string, foreground: string): string {
  const svg = [
    ...
    `<ellipse cx="12" cy="12" rx="8.8" ry="3.2" transform="rotate(-20 12 12)" stroke="${foreground}" stroke-width="1.7"/>`,
    `<circle cx="12" cy="12" r="4.1" stroke="${foreground}" stroke-width="1.8"/>`,
    `<circle cx="17.2" cy="7.7" r="1.05" fill="${foreground}"/>`,
    "</svg>",
  ].join("");
